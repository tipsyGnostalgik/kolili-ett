...
function tabs(index)
	...
	t[#t + 1] =
		...
		Name = "TabBG",
		InitCommand = function(self)
			self:y(2):valign(0):zoomto(frameWidth, 20):diffusecolor(getMainColor("frames")):diffusealpha(0)
		end,
	...
	t[#t + 1] =
		...
		{
			InitCommand = function(self)
				self:y(5):valign(0):zoom(0.4):diffuse(getMainColor("positive")):maxwidth(frameWidth * 2):diffusealpha(0)
			end,
			BeginCommand = function(self)
				self:queuecommand("Set"):diffusealpha(0)
			end,
			SetCommand = function(self)
				...
				if isTabEnabled(index) then
					if index == 6 and FILTERMAN:AnyActiveFilter() then
						self:diffuse(color("#cc2929")):diffusealpha(0)
					else
						self:diffuse(getMainColor("positive")):diffusealpha(0)
					end
				else
					self:diffuse(color("#666666")):diffusealpha(0)
				end
			end
		}
	return t
